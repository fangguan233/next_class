...
async function processData() {
    ...
    if (existingCourses && JSON.parse(existingCourses).length > 0) {
        ...
        const shouldContinue = await new Promise((resolve) => {
            showModal('确认覆盖', `
                <p>检测到已有课程数据，继续导入将覆盖现有课程表。</p>
            `, [
                { text: '取消', type: 'secondary', handler: () => { hideModal(); resolve(false); } },
                { text: '继续覆盖', type: 'primary', handler: () => { hideModal(); resolve(true); } }
            ]);
...
async function processTextData(button) {
    ...
    if (!importButton) {
        importButton = document.querySelector('button[onclick="processData()"]');
        if (!importButton) {
            const buttons = Array.from(document.querySelectorAll('button'));
            importButton = buttons.find(btn => btn.textContent.includes('导入数据') || btn.innerHTML.includes('导入数据'));
            if (!importButton) {
                throw new Error('无法找到导入按钮');
            }
        }
    }
    ...
    try {
        ...
        try {
            ...
            if (!btn) {
                btn = document.querySelector('button[onclick="processData()"]');
                if (!btn) {
                    const buttons = Array.from(document.querySelectorAll('button'));
                    btn = buttons.find(b => b.textContent.includes('导入数据') || b.innerHTML.includes('导入数据'));
                }
            }
            if (btn) {
                btn.disabled = false;
            ...
            console.warn("无法恢复按钮状态:", e.message);
        }
    }
}
...
async function processImageData(button) {
    ...
    try {
        ...
        const result = { success: true, courses: data.courses };

        if (!result.success) {
            // 如果有冲突，保存数据并跳转到编辑页面
            const normalizedCourses = normalizeCourses(result.courses).courses;
            localStorage.setItem('courses', JSON.stringify(normalizedCourses));
            sessionStorage.setItem('conflict_warning', result.message);
            showToast("检测到课程冲突！正在跳转到编辑页面...");
            setTimeout(() => {
                window.location.href = 'edit.html';
            }, 1500);
        } else {
            handleSuccessfulImport(result.courses);
        }
...
function handleFailedImportWithData(courses, message) {
    courses = normalizeCourses(courses).courses;
...
function handleSuccessfulImport(courses) {
    ...
    const normalizedCourses = normalizeCourses(courses).courses;
...
function goToNextClass() {
    ...
    const hasCourses = storedCourses && JSON.parse(storedCourses).length > 0;
    const hasExams = storedExams && JSON.parse(storedExams).length > 0;
    ...
    `, [
        { text: '导入', type: 'primary', handler: async () => {
            ...
            try {
                ...
                const normalizedCourses = normalizeCourses(data.courses).courses;
                localStorage.setItem('courses', JSON.stringify(normalizedCourses));
